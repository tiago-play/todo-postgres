...
//consulta se o usuario existe, caso não, insere o usuario
...
// cria todo com o id do usuario no parametro de url
...
// listar todos os toDos de um usuario
...
app.patch('/todo/:user_id/:todo_id', async (req, res) => {
    ...
    const data = req.body
    try {
        const belongsToUser = await pool.query('SELECT * FROM todos where user_id = ($1) and todo_id = ($2)', 
            [user_id, todo_id])
...
// moudlo cors permite que a api seja acessa por qualquer endereço
